...
void BP_KaniArm::PostUTRSUpdate() {	
	for (int i = 0; i < enLRNUM; i++) {
		//腕の目標点(IKターゲット)の更新
		m_ikSetting[i]->targetPos = TurnArm(m_isMachineGunning[i] ? m_ptrCore->GetTargetPos() : m_ptrCore->GetVanisingPoint(), m_model->GetBonePos(m_muzzleBoneID[i]), i != L ? m_ptrCore->GetLeft() : m_ptrCore->GetRight(), 110.0f, 10.0f, 0.2f, m_ikTargetPos[i]);
